...
const feature = loadFeature('../features/updateGroup.feature');

defineFeature(feature, (test) => {
  let response;

  test('deletar grupo', ({ given, when, then, and }) => {
    given(/^Existe no sistema um grupo com "id": "([^"]+)", "nome": "([^"]+)"$/, async (id, nome) => {
    ...
    when(/^uma requisição PUT é feita para "([^"]+)" com body "name": "([^"]+)"$/, async (route, nome) => {
    ...
    and(/^não existe no sistema um grupo com "id": "([^"]+)", "nome": "([^"]+)"$/, async (id, nome) => {
      const group = await (await connection.get()).groups.findUnique({where: {id}});
      if (!group) {
    ...
    and(/^existe no sistema um grupo com "id": "([^"]+)", "nome": "([^"]+)"$/, async (id, nome) => {
